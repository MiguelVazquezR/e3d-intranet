        ...
        <x-slot name="title">
            ...
            <ul class="flex justify-center items-center pb-4" x-data="setup()">
                <template x-for="(tab, index) in tabs" :key="index">
                    <li class="cursor-pointer text-sm py-2 px-6 text-gray-500 border-b-2" :class="activeTab===index ? 'text-black border-black' : ''" @click="activeTab = index; $dispatch('change-tab', index);" x-text="tab"></li>
                </template>
            </ul>
            <!--tab component end-->
            ...
            <div x-data="{activeTab: @entangle('active_tab')}" @change-tab.window="activeTab = $event.detail">
                ...
                <div x-show="activeTab == 0">
                    ...
                    <div class="lg:grid lg:grid-cols-4 lg:gap-x-2">
                        <div class="lg:col-span-3">
                            ...
                            <x-jet-input wire:model.defer="company.fiscal_address" type="text" class="w-full mt-2 placeholder:text-xs" placeholder="calle, colonia, # interior y/o exterior, ciudad, estado, país" />
                    ...
                    <div class="p-3 bg-sky-100 my-2 rounded-2xl">
                        ...
                        <x-item-list :index="$i" :active="array_key_exists('id', $contact) ? !in_array($contact['id'],$temporary_contacts_deleted_list) : true" :objectId="array_key_exists('id', $contact) ? $contact['id'] : null" inactiveMessage="Contacto eliminado de la sucursal" editMethodName="editContact" deleteMethodName="deleteContact">
                        ...
                        <div class="flex justify-end mt-3 p-1">
                            ...
                            @else
                            <div class="flex">
                                <div wire:click="updateBranchFromList" class="hover:cursor-pointer flex items-center">
                                    <i class="fas fa-check-circle text-green-600 mr-1"></i>
                                    <span class="text-xs text-green-500 mr-3">Actualizar Sucursal</span>
                                </div>
                                <div wire:click="closeEditBranch" class="hover:cursor-pointer flex items-center">
                                    <i class="fas fa-times"></i>
                                    <span class="text-xs text-gray-600 ml-1">Cancelar</span>
                                </div>
                            </div>
                    ...
                    <x-item-list :index="$i" :active="array_key_exists('id', $branch['branch']) ? !in_array($branch['branch']['id'],$temporary_branches_deleted_list) : true" :objectId="array_key_exists('id', $branch['branch']) ? $branch['branch']['id'] : null" inactiveMessage="Sucursal eliminada del cliente" editMethodName="editBranch" deleteMethodName="deleteBranch">
                        <x-product-quick-view name="{{$i+1}}:">
                ...
                <div x-show="activeTab == 1" class="text-gray-500">
                    <div class="flex justify-between pb-4 border-b-2">
                        ...
                        <x-jet-secondary-button wire:click="$emitTo('company-has-product-for-sell.create-company-has-product-for-sell', 'openModal', {{ $company }} )" class="ml-2">
                            Registrar producto
                        </x-jet-secondary-button>
                    </div>
                    ...
                    <div class="grid-cols-1 md:grid md:grid-cols-2 md:gap-3 mt-2 text-sm">
                        ...
                        <x-simple-product-card :simpleProduct="$product">
                            ...
                            <div class="grid grid-cols-2 gap-2 border-t-2 p-2">
                                ...
                                <p class="mt-1 text-gray-500">Precio anterior: <span class="text-green-600">{{ $product_for_sell->old_price . ' ' . $product_for_sell->old_price_currency }}</span> </p>
                                ...
                                @endif
                                <p class="mt-1 text-gray-500">Precio actual: <span class="text-green-600">{{ $product_for_sell->new_price . ' ' . $product_for_sell->new_price_currency }}</span> </p>
                                <p class="mt-1 text-gray-500">Establecido:
                                    <span class="text-sky-600">
                            ...
                            <div class="flex justify-end text-xs p-2">
                                <div wire:click="updatePriceProductForSell({{ $product_for_sell }})" class="mr-3 text-green-600 hover:cursor-pointer">
                                    <i class="fas fa-money-bill-wave"></i>
                                    <i class="fas fa-angle-double-up"></i>
                                </div>
                                <i wire:click="editProductForSell( {{$product_for_sell}} )" class="fas fa-edit mr-3 text-blue-500 hover:cursor-pointer"></i>
                                <i wire:click="deleteProductForSell({{ $product_for_sell }})" class="fas fa-trash-alt text-red-500 hover:cursor-pointer"></i>
                            </div>
