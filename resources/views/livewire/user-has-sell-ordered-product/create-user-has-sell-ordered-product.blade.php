        ...
        <x-slot name="content">
            <div class="grid grid-cols-2 gap-2">
                <div>
                    ...
                    <x-select class="mt-2 w-full" wire:model.defer="user_id">
                        <option value="" selected>-- Seleccione --</option>
                        @forelse($operators as $operator)
                        <option value="{{ $operator->id }}">{{ $operator->name }}</option>
                        @empty
                        <option value="">No hay ningun operador registrado</option>
                        @endforelse
                    </x-select>
                    <x-jet-input-error for="user_id" class="mt-3" />
                </div>
                <div>
                    ...
                    <x-jet-input-error for="estimated_time" class="mt-3" />
                </div>
            </div>
            <div class="mt-2">
                <x-jet-label value="Indicaciones" />
                <textarea wire:model.defer="indications" rows="3" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm w-full"></textarea>
                <x-jet-input-error for="indications" class="mt-3" />
            </div>
            @if( is_null($edit_index) )
            <div wire:click="addItemToList" class="hover:cursor-pointer flex justify-end items-center text-green-600 mt-3">
            ...
            <div class="flex justify-between border-b-2 py-3 mx-6 text-gray-500">
                ...
                <span wire:click="removeFromTemporaryDeletedList( {{$activities['id']}} )" class="text-sm text-blue-500 hover:cursor-pointer">
                    Deshacer
                </span>
            </div>
            @else
            <div class="flex items-center justify-between text-sm mb-1 py-2 mx-6 border-b-2">
                <p><i class="fas fa-user-circle mr-1"></i><span class="mr-2">{{ $operator->name }}</span></p>
                <div class="flex items-center">
                    <i wire:click="editItem({{ $i }})" class="fas fa-edit mr-3 text-blue-500 hover:cursor-pointer"></i>
                    <i wire:click="deleteItem({{ $i }})" class="fas fa-trash-alt text-red-500 hover:cursor-pointer"></i>
                </div>
            </div>
            @endif
            @else
            ...
            @endif
            @endforeach
            <x-jet-input-error for="activities_detail_list" class="mt-2" />
        </x-slot>
