        ...
        <x-slot name="content">
            <div x-data="{ activeTab: @entangle('active_tab') }" @change-tab.window="activeTab = $event.detail">
                ...
                <div x-show="activeTab == 0">
                    ...
                    <div class="lg:grid lg:grid-cols-2 lg:gap-2 font-bold">
                        ...
                        @if ($sell_order->received_at)
                            <p>fecha de recibido: <span
                                    class="text-gray-500">{{ $sell_order->received_at->isoFormat('D MMMM YYYY') }}</span>
                            </p>
                        @endif
                    </div>
                    @if ($sell_order->customer)
                        <h3 class="text-center text-lg text-sky-800 tracking-widest font-bold my-2">Datos del clinte</h3>
                        <div class="lg:grid lg:grid-cols-2 lg:gap-2 font-bold">
                            <p>Razón social: <span
                                    class="text-gray-500">{{ $sell_order->customer->company->bussiness_name }}</span>
                            </p>
                            <p>RFC: <span class="text-gray-500">{{ $sell_order->customer->company->rfc }}</span></p>
                            ...
                            <p>Medio de pago: <span class="text-gray-500">{{ $sell_order->customer->satWay->key }} -
                                    {{ $sell_order->customer->satWay->description }}</span></p>
                            <p>Uso de factura: <span class="text-gray-500">{{ $sell_order->customer->satType->key }}
                    ...
                    <h3 class="text-center text-lg text-sky-800 tracking-widest font-bold my-2">Datos de la orden</h3>
                    <div class="lg:grid lg:grid-cols-2 lg:gap-2 font-bold">
                        <p>Solicitada por: <span class="text-gray-500">{{ $sell_order->creator->name }}</span></p>
                        <p>Solicitada el: <span
                                class="text-gray-500">{{ $sell_order->creator->created_at->isoFormat('D MMMM YYYY') }}</span>
                        </p>
                            ...
                            @if ($s_o_p->productForSell->model_name == 'App\\Models\\' . Product::class)
                                ...
                                <x-simple-product-card :simpleProduct="$product">
                                    ...
                                    <div class="mt-1 text-gray-700 border-t-2 pt-2">Operadores asignados:
                                        @php
                                            $current_operator_activity = null;
                                        @endphp
                                        @forelse($s_o_p->activityDetails as $activity_details)
                                            @php
                                                if ($activity_details->operator->id == Auth::user()->id) {
                                                    $current_operator_activity = $activity_details;
                                                }
                                            @endphp
                                            <li class="text-gray-500 flex justify-between items-center">
                                                ...
                                                <div x-data="{ open_tooltip: false }" class="flex items-center">
                                                    <span @click="open_tooltip = !open_tooltip"
                                                        ...
                                                        <div @click.away="open_tooltip=false" x-show="open_tooltip"
                                                            x-transition
                                                            ...
                                                            <div
                                                                ...
                                                                {{ $activity_details->estimated_time }} minutos
                                                                <br><br>
                                    ...
                                    @if ($current_operator_activity)
                                        <div>
                                            @if ($current_operator_activity->finish)
                                                <span
                                            ...
                                            @elseif($current_operator_activity->start)
                                                @if ($current_operator_activity->pause)
                                                    <span
                                                    ...
                                                    <button wire:click="continue({{ $current_operator_activity }})"
                                                        class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-3 py-1 text-center">
                                                        Continuar
                                                    </button>
                                                @else
                                                    <button wire:click="pause({{ $current_operator_activity }})"
                                                        class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-3 py-1 text-center">
                                                        Pausar
                                                    </button>
                                                    <button wire:click="finish({{ $current_operator_activity }})"
                                                        class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-3 py-1 text-center">
                                                        Finalizar
                                                    </button>
                                                @endif
                                            @else
                                                <button wire:click="start({{ $current_operator_activity }})"
                                                    class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-3 py-1 text-center">
                                                    Iniciar
                                                </button>
                                            @endif
                                ...
                                <x-composit-product-card :compositProduct="$product">
                                    ...
                                    <div class="grid grid-cols-2 gap-2 border-t-2 p-2">
                                        @if ($s_o_p->productForSell->old_price)
                                            ...
                                            <p class="mt-1 text-gray-500">Establecido:
                                                <span class="text-sky-600">
                                                    @if ($s_o_p->productForSell->old_date->diffForHumans() == 'hace 6 horas')
                                                        {{ 'hoy a las ' . $s_o_p->productForSell->old_date->isoFormat('h:mm a') }}
                                                    @else
                                                        {{ $s_o_p->productForSell->old_date->diffForHumans() }}
                                                    @endif
                                        ...
                                        <p class="mt-1 text-gray-500">Establecido:
                                            <span class="text-sky-600">
                                                @if ($s_o_p->productForSell->new_date->diffForHumans() == 'hace 6 horas')
                                                    {{ 'hoy a las ' . $s_o_p->productForSell->new_date->isoFormat('h:mm a') }}
                                                @else
                                                    {{ $s_o_p->productForSell->new_date->diffForHumans() }}
                                                @endif
