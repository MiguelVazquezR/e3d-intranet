<div>
    @if ($open)
        <x-jet-dialog-modal wire:model="open">
                ...
                <ul class="flex justify-center items-center pb-4" x-data="{
                    ...
                    <template x-for="(tab, index) in tabs" :key="index">
                        <li class="cursor-pointer text-sm py-2 px-6 text-gray-500 border-b-2"
                            ...
                            @click="activeTab = index; changeAction(); $dispatch('change-tab-cp-show', index);"
                            x-text="tab"></li>
                    </template>
                </ul>
                <!--tab component end-->
            ...
            <x-slot name="content">
                <div x-data="{ activeTab: @entangle('active_tab'), action: @entangle('action') }" @change-tab-cp-show.window="activeTab = $event.detail">
                    <!-- Detalles -->
                    <div x-show="activeTab==0">
                        <h2 class="mb-4 text-gray-500">
                            Producto compuesto de inventario <span class="text-black">{{ $stock_product->id }}</span>
                        </h2>

                        <div class="grid grid-cols-3 gap-4">
                            @if ($stock_product->image)
                                <img class="w-48 h-48 rounded-2xl object-fill"
                                    src="{{ Storage::url($stock_product->image) }}">
                            @endif
                            @if ($stock_product->compositProduct)
                                <div>
                                    <x-jet-label value="Producto" class="mt-3" />
                                    <x-product-quick-view :image="$stock_product->compositProduct->image" :name="$stock_product->compositProduct->alias" />
                                    <div>
                                        <x-jet-label value="Cantidad actual" class="mt-3" />
                                        <p>{{ $stock_product->quantity . ' unidades' }}</p>
                                    </div>
                                </div>
                                <div>
                                    <div class="mb-4">
                    ...
                    <div x-show="activeTab==1" class="text-gray-500">
                        <div class="flex justify-between pb-4 border-b-2">
                            @if ($stock_product->compositProduct)
                                <h2>
                                    Entradas de <span
                                        class="text-black font-extrabold">{{ $stock_product->compositProduct->alias }}</span>
                                </h2>
                            @endif
                        </div>
                                ...
                                <table>
                                    <thead>
                                        <tr class="text-xs uppercase text-gray-700">
                                            ...
                                            <td wire:click="$set('human_format', {{ !$human_format }})"
                                                class="w-1/5 p-2 hover:cursor-pointer">Fecha <i
                                                    class="fas fa-exchange-alt"></i></td>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($stock_movements as $movement)
                                            <tr class="text-xs my-2 bg-green-100 border-b border-green-300">
                                                ...
                                                @if ($human_format)
                                                    <td class="w-1/5 p-1">
                                                        {{ $movement->created_at->diffForHumans() }} </td>
                                                @else
                                                    <td class="w-1/5 p-1">
                                                        {{ $movement->created_at->isoFormat('DD MMMM YYYY h:mm a') }}
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
            ...
            <table>
                <thead>
                    <tr class="text-xs uppercase text-gray-700">
                        ...
                        <td class="w-1/5 p-2">Notas</i></td>
                        <td wire:click="$set('human_format', {{ !$human_format }})"
                            class="w-1/5 p-2 hover:cursor-pointer">Fecha <i class="fas fa-exchange-alt"></i></td>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($stock_movements as $movement)
                        <tr class="text-xs my-2 bg-red-100 border-b border-red-300">
                            ...
                            @else
                                <td class="w-1/5 p-1"> {{ $movement->created_at->isoFormat('DD MMMM YYYY h:mm a') }}
                                </td>
                            @endif
                        </tr>
                    @endforeach
