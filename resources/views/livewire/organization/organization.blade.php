...
<div class="mx-4 lg:mx-20 mt-10 lg:grid lg:grid-cols-2 lg:gap-x-3">
    <div class="lg:border-r-2 px-2">
        <div>
            ...
            <x-jet-input wire:model.defer="organization.name" type="text" class="w-full input" />
            <x-jet-input-error for="organization.name" class="text-xs" />
        </div>

        <div>
            <x-jet-label value="Razón social" class="mt-3 dark:text-gray-400" />
            <x-jet-input wire:model.defer="organization.bussiness_name" type="text" class="w-full input" />
            <x-jet-input-error for="organization.bussiness_name" class="text-xs" />
        </div>

        <div>
            <x-jet-label value="RFC" class="mt-3" />
            <x-jet-input wire:model.defer="organization.rfc" type="text" class="w-full input" />
            <x-jet-input-error for="organization.rfc" class="text-xs" />
        </div>

        <div class="grid grid-cols-2 gap-x-2">
            <div>
                <x-jet-label value="Teléfono 1" class="mt-3 dark:text-gray-400" />
                <x-jet-input wire:model.defer="organization.phone1" type="text" class="w-full input" />
                <x-jet-input-error for="organization.phone1" class="text-xs" />
            </div>
            <div>
                <x-jet-label value="Teléfono 2" class="mt-3 dark:text-gray-400" />
                <x-jet-input wire:model.defer="organization.phone2" type="text" class="w-full input" />
                <x-jet-input-error for="organization.phone2" class="text-xs" />
            </div>
        </div>

        <div>
            <x-jet-label value="Sitio web" class="mt-3 dark:text-gray-400" />
            <x-jet-input wire:model.defer="organization.web_site" type="text" class="w-full input" />
        ...
        <div class="grid grid-cols-4 gap-x-2">
            <div class="col-span-3">
                ...
                <x-jet-input wire:model.defer="organization.address" type="text" class="w-full input" />
                <x-jet-input-error for="organization.address" class="text-xs" />
            </div>
            <div>
                <x-jet-label value="C.P." class="mt-3 dark:text-gray-400" />
                <x-jet-input wire:model.defer="organization.post_code" type="number" class="w-full input" />
    ...
    {{-- images --}}
    <div class="px-2 mt-3">
        <x-image-uploader :image="$logo" :imageExtensions="$image_extensions" :imageId="$logo_image_id" label="Logo de la organización" class="dark:text-gray-400"
        ...
        <x-image-uploader :image="$shield" :imageExtensions="$image_extensions" :imageId="$shield_image_id" :showAlerts="false"
            label="Escudo de la organización" model="shield" :registeredImage="$organization->shield ?? null" class="mt-4 dark:text-gray-400" />
    </div>
