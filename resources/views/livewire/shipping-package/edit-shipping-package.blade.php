    ...
    <x-jet-dialog-modal wire:model="open">
        ...
        <x-slot name="content">
            @if ($sell_ordered_product)
                ...
                @foreach ($packages_list as $i => $package)
                    <x-item-list :index="$i" :active="array_key_exists('id', $package)
                        ? !in_array($package['id'], $temporary_deleted_list)
                        : true" :objectId="array_key_exists('id', $package) ? $package['id'] : null" :inactiveMessage="$deleted_message"
                        :canUndo="true" :edit="false">
        ...
        <x-slot name="footer">
            ...
            <x-jet-button wire:click="update" wire:loading.attr="disabled"
                wire:target="update,inside_image,outside_image" class="disabled:opacity-25">
                Actualizar
            </x-jet-button>
        </x-slot>
    </x-jet-dialog-modal>
