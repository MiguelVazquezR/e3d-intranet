<div>
    ...
    <div class="py-6">
        ...
        <div wire:loading wire:target="edit,show,resetPassword,activitiesReport">
            <x-loading-indicator />
        </div>
        ...
        <div class="w-11/12 lg:w-3/4 mx-auto">
            <x-jet-input class="w-full placeholder:text-xs input" wire:model="search" type="text" name="search"
                placeholder="Escribe el ID, nombre o correo" />
        </div>
                    ...
                    <tr>
                            ...
                            @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                                <div
                                    ...
                                    <a class="hover:cursor-pointer" target="_blank"
                                        href="{{ $item->profile_photo_url }}">
                                        <img class="h-10 w-10 rounded-full object-cover border-2"
                                            src="{{ $item->profile_photo_url }}" alt="{{ Auth::user()->name }}" />
                                    </a>
                                    <p class="text-gray-900 whitespace-no-wrap ml-1 dark:text-gray-400">
                                        {{ $item->name }}
                                    </p>
                                </div>
                            @else
                        ...
                        <td class="w-28 px-px py-3 border-b dark:border-slate-600 dark:bg-slate-700 border-gray-200 bg-white">
                            ...
                            @can('eliminar_usuarios')
                                <i wire:click="$emit('confirm', {0:'user.users', 1:'delete' ,2:{{ $item->id }}, 3:'Este proceso no se puede revertir'})"
                                    class="fas fa-trash bg-red-500 text-white p-2 mt-1 rounded-lg ml-px hover:cursor-pointer"></i>
                            @endcan
                        </td>
                        <td class="py-3 border-b dark:border-slate-600 dark:bg-slate-700 border-gray-200 bg-white text-center">
                            @can('más_acciones_usuarios')
                                <x-jet-dropdown align="right" width="48">
                                    ...
                                    <x-slot name="content">
                                        <x-jet-dropdown-link href="{{ route('vacations-receipt-format', $item) }}"
                                            target="_blank">
                                            Generar recibo de vacaciones
                                        </x-jet-dropdown-link>
                                        <x-jet-dropdown-link wire:click="resetPassword( {{ $item }} )"
                                            :link="false">
                                            Resetear contraseña
                                        </x-jet-dropdown-link>
                                        @if ($item->hasRole('Auxiliar_producción'))
                                            <x-jet-dropdown-link wire:click="activitiesReport( {{ $item }} )"
                                                :link="false">
        ...
        @livewire('user.activities-report')
