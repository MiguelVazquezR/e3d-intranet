    ...
    <x-jet-dialog-modal wire:model="open">
        ...
        <x-slot name="content">
            ...
            <h2 class="text-center font-bold text-lg text-sky-600 mt-3 flex items-center justify-center">
                Resultados
                <i wire:click="addDesignResult" class="fas fa-plus-circle text-green-600 hover:cursor-pointer ml-3"></i>
            </h2>
            <x-jet-input-error for="design_results_list" class="mt-2" />
            @foreach($design_results_list as $i => $design_result)
            <x-item-list :index="$i" :active="true" :edit="false">
            ...
            <div class="col-span-2">
                ...
                <x-jet-input-error for="tentative_end" class="mt-3" />
            </div>
            @endif
            @endif
        </x-slot>
        ...
        <x-slot name="footer">
            ...
            @if($design_order->id)
            @if(!$design_order->tentative_end)
            <x-jet-button wire:click="storeTentativeEnd" wire:loading.attr="disabled" wire:target="storeTentativeEnd" class="disabled:opacity-25">
